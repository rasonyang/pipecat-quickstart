...
#### WebRTC Transport (Browser-based)
...
To accept incoming SIP calls via telephone:

```bash
uv run bot.py --transport sip
```

Configure SIP settings in `.env`:
```bash
SIP_SERVER_HOST=0.0.0.0        # Bind address
SIP_SERVER_PORT=6060           # SIP signaling port
SIP_RTP_PORT_RANGE=10000-15000 # RTP audio port range
```

**Network Requirements:**
- UDP port 6060 open for SIP signaling
- UDP ports 10000-15000 open for RTP audio
- Configure your SIP trunk or client to send calls to `<server-ip>:6060`

**Supported Codec:** G.711 μ-law (PCMU) at 8kHz
